...
class Settings:
    ...
    try:
        CACHE_ROOT = (
            os.path.join(HOMEDIR, ".cache", "SilverDict")
            if HOMEDIR
            else "/tmp/SilverDict"
        )
        APP_RESOURCES_ROOT = (
            os.path.join(HOMEDIR, ".silverdict") if HOMEDIR else "/tmp/SilverDict"
        )
    ...
    DICTIONARY_METADATA_FILE = os.path.join(
        APP_RESOURCES_ROOT, "dictionary_metadata.yaml"
    )
    # [
    # 	{
    # 		"dictionary_name": "oxford_dictionary_of_english",
    ...
    FORGETTING_CURVE_FILE = os.path.join(APP_RESOURCES_ROOT, "curve.yaml")
    ...
    def _save_dictionary_list(self) -> None:
        ...
        for dictionary_info in self.dictionaries_list:
            if dictionary_info[
                "dictionary_format"
            ] == "DSL (.dsl/.dsl.dz)" and not dictionary_info[
                "dictionary_filename"
            ].endswith(
                ".dz"
            ):
        ...
        if sys.platform != "win32":  # FIXME: ensure other platforms do use ~
            ...
            for dictionary_info in dictionaries_list:
                if dictionary_info["dictionary_filename"].startswith(self.HOMEDIR):
                    dictionary_info["dictionary_filename"] = dictionary_info[
                        "dictionary_filename"
                    ].replace(self.HOMEDIR, "~")
            self._save_settings_to_file(dictionaries_list, self.DICTIONARIES_LIST_FILE)
        else:
            self._save_settings_to_file(
                self.dictionaries_list, self.DICTIONARIES_LIST_FILE
            )

    def _save_dictionary_metadata(self) -> None:
        self._save_settings_to_file(
            self.dictionary_metadata, self.DICTIONARY_METADATA_FILE
        )
    ...
    def _save_history(self) -> None:
        ...
        self._save_settings_to_file(self.forgetting_curve, self.FORGETTING_CURVE_FILE)
    ...
    def change_suggestions_mode_from_right_side_to_both_sides(self) -> None:
        ...
        preferences = preferences.replace(
            "suggestions_mode: right-side", "# suggestions_mode: right-side"
        )
        preferences = preferences.replace(
            "# suggestions_mode: both-sides", "suggestions_mode: both-sides"
        )
...
full_text_search_diacritic_insensitive: false"""
        ...
        self.preferences: dict[str, str] = self._read_settings_from_file(
        ...
        if os.path.isfile(self.DICTIONARIES_LIST_FILE):
            self.dictionaries_list: list[dict[str, str]] = (
                self._read_settings_from_file(self.DICTIONARIES_LIST_FILE)
            )
            for dictionary_info in self.dictionaries_list:
                if (
                    "$" in dictionary_info["dictionary_filename"]
                    or "~" in dictionary_info["dictionary_filename"]
                    or "%" in dictionary_info["dictionary_filename"]
                ):
                    dictionary_info["dictionary_filename"] = (
                        self.parse_path_with_env_variables(
                            dictionary_info["dictionary_filename"]
                        )
                    )
                ...
                if not os.path.isfile(dictionary_info["dictionary_filename"]):
                    logger.warning(
                        f'Dictionary {dictionary_info["dictionary_name"]} not found, removing from the list.'
                    )
        ...
        if os.path.isfile(self.GROUPS_FILE):
            self.groups: list[dict[str, str | set[str]]] = (
                self._read_settings_from_file(self.GROUPS_FILE)
            )
        else:
        ...
        if os.path.isfile(self.JUNCTION_TABLE_FILE):
            self.junction_table: dict[str, set[str]] = self._read_settings_from_file(
                self.JUNCTION_TABLE_FILE
            )
            for dictionary_info in self.dictionaries_list:
                if not dictionary_info["dictionary_name"] in self.junction_table.keys():
                    self.junction_table[dictionary_info["dictionary_name"]] = {
                        "Default Group"
                    }
        ...
        if os.path.isfile(self.HISTORY_FILE):
            self.lookup_history: list[str] = self._read_settings_from_file(
                self.HISTORY_FILE
            )
            self.forgetting_curve: list[int] = self._read_settings_from_file(
                self.FORGETTING_CURVE_FILE
            )
        else:
            self.lookup_history: list[str] = []
            self.forgetting_curve: list[int] = []
            self._save_history()

        if os.path.isfile(self.MISC_CONFIGS_FILE):
            self.misc_configs: dict = self._read_settings_from_file(
                self.MISC_CONFIGS_FILE
            )
    ...
    def dictionary_info_valid(self, dictionary_info: dict) -> bool:
        ...
        return (
            ...
            and dictionary_info["dictionary_format"]
            in self.SUPPORTED_DICTIONARY_FORMATS.keys()
            and os.access(filename, os.R_OK)
            and os.path.isfile(filename)
            and os.path.splitext(filename)[1]
            in self.SUPPORTED_DICTIONARY_FORMATS[dictionary_info["dictionary_format"]]
            and not any(
                dictionary_info["dictionary_name"] == dictionary["dictionary_name"]
                for dictionary in self.dictionaries_list
            )
        )

    def group_valid(self, group: dict[str, str | set[str]]) -> bool:
        return all(key in group.keys() for key in ["name", "lang"]) and all(
            lang in self.LANGS for lang in group["lang"]
        )
    ...
    @_safe
    def add_dictionary(
        self, dictionary_info: dict, groups: list[str] | None = None
    ) -> None:
        ...
        self._save_dictionary_metadata()
        self.junction_table[dictionary_info["dictionary_name"]] = (
            groups if groups else {"Default Group"}
        )
        self._save_junction_table()
    ...
    @_safe
    def change_dictionary_display_name(
        self, dictionary_name: str, new_dictionary_display_name: str
    ) -> None:
        for dictionary_info in self.dictionaries_list:
            if dictionary_info["dictionary_name"] == dictionary_name:
                dictionary_info["dictionary_display_name"] = new_dictionary_display_name
                logger.info(
                    f'Name of dictionary {dictionary_info["dictionary_name"]} '
    ...
    def remove_dictionary(self, dictionary_info: str) -> None:
        ...
        resources_dir = os.path.join(
            self.CACHE_ROOT, dictionary_info["dictionary_name"]
        )
        ...
        if os.path.isfile(
            os.path.join(self.CACHE_ROOT, dictionary_info["dictionary_name"] + ".syn")
        ):
            # StarDict .syn file converted to pickle
            os.remove(
                os.path.join(
                    self.CACHE_ROOT, dictionary_info["dictionary_name"] + ".syn"
                )
            )
    ...
    @_safe
    def update_dictionary_modification_time(
        self, dictionary_name: str, new_time: float
    ) -> None:
        for m in self.dictionary_metadata:
            if m["dictionary_name"] == dictionary_name:
    ...
    def get_groups(self) -> list[dict[str, str | list[str]]]:
        return [
            {"name": group["name"], "lang": list(group["lang"])}
            for group in self.groups
        ]
    ...
    def change_group_lang(self, group_name: str, new_group_lang: list[str]) -> None:
        for group in self.groups:
            if group["name"] == group_name:
                ...
                logger.info(
                    f'Languages of group {group_name} changed to {group["lang"]}.'
                )
                self._save_groups()
                break
    ...
    def reorder_groups(self, groups: list[dict[str, str | list[str]]]) -> None:
        ...
        # Convert list to set
        groups = [
            {"name": group["name"], "lang": set(group["lang"])} for group in groups
        ]
    ...
    def reorder_dictionaries(self, dictionaries_info: list[dict[str, str]]) -> None:
        ...
        if not all(
            dictionary_info in dictionaries_info
            for dictionary_info in self.dictionaries_list
        ):
    ...
    @_safe
    def remove_dictionary_from_group(
        self, dictionary_name: "str", group_name: "str"
    ) -> "None":
        self.junction_table[dictionary_name].remove(group_name)
        self._save_junction_table()
    ...
    def scan_source(self, source: str) -> Generator[dict[str, str], None, None]:
        for filename in os.listdir(source):
            ...
            if (
                os.path.isfile(full_filename) and filename.find("_abrv.dsl") == -1
            ):  # see dsl_reader.py
                dictionary_format = self._dictionary_format(full_filename)
                if dictionary_format:
                    if not any(
                        dictionary_info["dictionary_filename"] == full_filename
                        for dictionary_info in self.dictionaries_list
                    ):
                        ...
                        logger.info(
                            f"Found dictionary {name} ({full_filename}) during re-scanning."
                        )
                        yield {
    ...
    def add_to_history(self, word: str) -> None:
        # this is a valid word, and should be put into histories if searched for the first time

        if word not in self.lookup_history:
            self.lookup_history.insert(0, word)
            self.forgetting_curve.insert(0, 0)
    ...
    @_safe
    def forgetting_curve_lookup(self, frequent: int) -> list[str]:
        indexes = []
        index = 0
        frequent = int(frequent)
        print(self.forgetting_curve)
        for word in self.forgetting_curve:
            print(word, frequent)
            if word == frequent:
                self.forgetting_curve[index] += 1
                indexes.append(index)
            index += 1
        print(indexes)
        words = []
        print(self.lookup_history)
        for index in indexes:
            print(index)
            words.append(self.lookup_history[index])
        print(words)
        return words
