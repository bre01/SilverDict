...
class Dictionaries:
    _re_illegal_css_selector_chars = re.compile(
        "[\\~!@\\$%\\^\\&\\*\\(\\)\\+=,\\./';:\"\\?><\\[\\]\\\\\\{\\}\\|`\\#]"
    )
    ...
    def _load_dictionary(self, dictionary_info: dict) -> None:
        ...
        if self.settings.preferences["running_mode"] != "server":
            prev_time_modified = self.settings.saved_dictionary_modification_time(
            ...
            if prev_time_modified and prev_time_modified < cur_time_modified:
                ...
                logger.info(
                    f'Entries of {dictionary_info["dictionary_display_name"]} deleted from database,'
                    "ready for re-indexing."
                )

        match dictionary_info["dictionary_format"]:
            case "MDict (.mdx)":
                self._dictionaries[dictionary_info["dictionary_name"]] = MDictReader(
                    dictionary_info["dictionary_name"],
            ...
            case "StarDict (.ifo)":
                self._dictionaries[dictionary_info["dictionary_name"]] = StarDictReader(
                    dictionary_info["dictionary_name"],
                    dictionary_info["dictionary_filename"],
                    dictionary_info["dictionary_display_name"],
            ...
            case "DSL (.dsl/.dsl.dz)":
                if self.settings.preferences["running_mode"] == "normal":
                    self._dictionaries[dictionary_info["dictionary_name"]] = DSLReader(
                        dictionary_info["dictionary_name"],
                        dictionary_info["dictionary_filename"],
                        dictionary_info["dictionary_display_name"],
                        load_content_into_memory=self.settings.dictionary_is_in_group(
            ...
            case _:
                raise ValueError(
                    f'Dictionary format {dictionary_info["dictionary_format"]} not supported.'
                )
        ...
        if self.settings.preferences["running_mode"] != "server":
            ...
            cur_time_modified = os.path.getmtime(dictionary_info["dictionary_filename"])
            if prev_time_modified and prev_time_modified < cur_time_modified:
                self.settings.update_dictionary_modification_time(
                    dictionary_info["dictionary_name"], cur_time_modified
                )
    ...
    def __init__(self, app: Flask) -> None:
        ...
        if (
            len(
                self.settings.dictionaries_of_group(
                    Settings.NAME_GROUP_LOADED_INTO_MEMORY
                )
            )
            > 0
    ...
    def add_dictionary(self, dictionary_info: dict) -> None:
        dictionary_info["dictionary_name"] = self._re_illegal_css_selector_chars.sub(
            "", dictionary_info["dictionary_name"]
        )
        # Remove whitespace and prepend '__' to make it a valid CSS selector
        dictionary_info["dictionary_name"] = "__" + "".join(
            dictionary_info["dictionary_name"].split()
        )

        dictionary_info["dictionary_filename"] = (
            self.settings.parse_path_with_env_variables(
                dictionary_info["dictionary_filename"]
            )
        )
        ...
        if any(
            d["dictionary_filename"] == dictionary_info["dictionary_filename"]
            for d in self.settings.dictionaries_list
        ):
            raise ValueError("Duplicate dictionary file.")

        while any(
            d["dictionary_name"] == dictionary_info["dictionary_name"]
            for d in self.settings.dictionaries_list
        ):
    ...
    def reload_dictionaries(self, dictionaries_info: list[dict]) -> None:
        ...
        added_dictionaries = [
            dictionary_info
            for dictionary_info in dictionaries_info
    ...
    def get_spelling_suggestions(self, group_name: str, key: str) -> list[str]:
        ...
        suggestions = [
            ...
            for suggestion in spelling_suggestions(
                key, self.settings.group_lang(group_name)
            )
            if db_manager.entry_exists_in_dictionaries(
                simplify(suggestion), names_dictionaries_of_group
            )
        ]
        return db_manager.select_entries_with_keys(
            suggestions,
    ...
    def recreate_xapian_index(self) -> None:
        if not xapian_found:
            logger.warning(
                "Refusing to build xapian index without installing the module."
            )
            return
        ...
        with self._xapian_indexing_lock:
            ...
            xapian_db = xapian.WritableDatabase(
                f"{self.settings.XAPIAN_DIR}_part", xapian.DB_CREATE_OR_OPEN
            )
            indexer = xapian.TermGenerator()
            indexer.set_flags(xapian.TermGenerator.FLAG_CJK_NGRAM)

            for dict_name in self.settings.dictionaries_of_group(
                self.settings.XAPIAN_GROUP_NAME
            ):
                for word in db_manager.select_words_of_dictionary(dict_name):
                    ...
                    if self.settings.preferences[
                        "full_text_search_diacritic_insensitive"
                    ]:
    ...
    def full_text_search(self, query_str: str) -> list[tuple[str, str, str, str]]:
        ...
        def extract_article(m: xapian.MSetItem) -> None:
            ...
            dict_name, word = (
                m.document.get_data()
                .decode("utf-8")
                .split(self._XAPIAN_DICTNAME_WORD_SEP)
            )
            article = self._dictionaries[dict_name].get_definition_by_word(word)
            if article:
                if "zh" in group_lang:
                    article = self._safely_convert_chinese_article(article)
                article = self._re_legacy_lookup_api.sub(
                    replace_legacy_lookup_api, article
                )
                if dict_name in transformation.transform.keys():
                    article = transformation.transform[dict_name](article)
                if (
                    not autoplay_found
                    and (pos_autoplay := article.find("autoplay")) != -1
                ):
                    autoplay_found = True
                    # Only preserve the first autoplay
                    pos_autoplay += len("autoplay")
                    article = article[:pos_autoplay] + article[pos_autoplay:].replace(
                        "autoplay", ""
                    )
                    articles.append(
                        (
        ...
        articles = [
            (article[1], article[2], article[3], article[4]) for article in articles
        ]
    ...
    def _safely_convert_chinese_article(self, article: str) -> str:
        ...
        article = convert_chinese(
            article, self.settings.preferences["chinese_preference"]
        )
        for match in matches:
            article = article.replace(
                self._REPLACEMENT_TEXT, "/api/cache/%s/%s" % match, 1
            )
        return article
    ...
    def suggestions(self, group_name: str, key: str) -> list[str]:
        ...
        if any(
            wildcard in key_simplified for wildcard in self.settings.WILDCARDS.keys()
        ):
            # If key has any wildcards, search as is
            key_simplified = Settings.transform_wildcards(key_simplified)
            suggestions = db_manager.select_entries_like(
                key_simplified,
                names_dictionaries_of_group,
                self.settings.misc_configs["num_suggestions"],
            )
            ...
            for key_simplified_transliterated in keys + [key]:
                key_orthographic_forms = [
                    w
                    for w in orthographic_forms(
                        key_simplified_transliterated, group_lang
                    )
                    if any(
                        db_manager.entry_exists_in_dictionaries(
                            simplify(w_stem), names_dictionaries_of_group
                        )
                        for w_stem in stem(w, group_lang)
                    )
                ]
            ...
            suggestions.extend(
                db_manager.select_entries_beginning_with(
                    keys,
                    names_dictionaries_of_group,
                    suggestions,
                    self.settings.misc_configs["num_suggestions"],
            ...
            if (
                self.settings.preferences["suggestions_mode"] == "both-sides"
                and len(suggestions) < self.settings.misc_configs["num_suggestions"]
            ):
                keys_expanded = []
                for key_simplified in keys:
                    keys_expanded.extend(
                        db_manager.expand_key(
                            key_simplified,
                            self.settings.preferences["ngram_stores_keys"],
                        )
                    )
                suggestions.extend(
                    db_manager.select_entries_with_keys(
                        keys_expanded,
                        names_dictionaries_of_group,
                        suggestions,
                        self.settings.misc_configs["num_suggestions"],
    ...
    def forgetting(self, key: int) -> list[str]:
        return self.settings.forgetting_curve_lookup(key)
    ...
    def query(self, group_name: str, key: str) -> list[tuple[str, str, str]]:
        ...
        names_dictionaries_of_group = self.settings.dictionaries_of_group(group_name)
        group_lang = self.settings.group_lang(group_name)
        keys = [simplify(s) for s in stem(key, group_lang)] + self._transliterate_key(
            key_simplified, group_lang
        )
        keys = list(set(keys))
        autoplay_found = False
        articles = []
        ...
        def extract_articles_from_dictionary(dictionary_name: str) -> None:
            ...
            article = self._dictionaries[dictionary_name].get_definitions_by_keys(
                keys_found
            )
            if article:
                if "zh" in group_lang:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            len(names_dictionaries_of_group)
        ) as executor:
    ...
    def query_anki(self, group_name: str, word: str) -> list[tuple[str, str]]:
        ...
        def extract_article_from_dictionary(dictionary_name: "str") -> "None":
            if db_manager.headword_exists_in_dictionary(word, dictionary_name):
                article = self._dictionaries[dictionary_name].get_definition_by_word(
                    word
                )
                if article:
