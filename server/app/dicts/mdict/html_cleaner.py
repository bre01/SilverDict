...
class HTMLCleaner:
    ...
    def __init__(
        self, filename: str, dict_name: str, resources_dir: str, styles: str = ""
    ) -> None:
    ...
    def _fix_file_path(self, definition_html: str, file_extension: str) -> str:
        ...
        while (
            extension_position := definition_html.find(
                file_extension, extension_position
            )
        ) != -1:
            filename_position = definition_html.rfind('"', 0, extension_position) + 1
            filename = definition_html[
                filename_position : extension_position + len(file_extension)
            ]
            ...
            if not os.path.isfile(new_file_path_on_disk):
                if os.path.isfile(file_path_on_disk):
                    ...
                    definition_html = (
                        definition_html[:filename_position]
                        + self._href_root_dir
                        + definition_html[filename_position:]
                    )
            else:
                if os.path.isfile(file_path_on_disk):
                    if os.path.getmtime(file_path_on_disk) > os.path.getmtime(
                        new_file_path_on_disk
                    ):
                        shutil.copy(file_path_on_disk, new_file_path_on_disk)
                definition_html = (
                    definition_html[:filename_position]
            ...
            if filename.endswith(".css") or filename.endswith(".CSS"):
                ...
                while current_pos < len(css):
                    ...
                    if ch == "@":
                        n = current_pos
                        if (
                            css[current_pos : current_pos + 7].lower() == "@import"
                            or css[current_pos : current_pos + 10].lower()
                            == "@font-face"
                            or css[current_pos : current_pos + 10].lower()
                            == "@namespace"
                            or css[current_pos : current_pos + 8].lower() == "@charset"
                        ):
                            # Copy rule as is.
    ...
    def _flatten_nested_a(self, definition_html: str, depth: int) -> str:
        ...
        if depth == 0:
            ...
            a_closing_tag_pos = 0
            while (
                a_tag_start_pos := definition_html.find("<a", a_closing_tag_pos)
            ) != -1:
                a_tag_end_pos = definition_html.find(">", a_tag_start_pos)
                inner_html_start_pos = definition_html.find(">", a_tag_end_pos + 1) + 1
                if (
                    a_closing_tag_pos := definition_html.find(
                        "</a>", a_tag_end_pos, inner_html_start_pos
                    )
                ) != -1:
                    continue
                ...
                if definition_html.find("href", a_tag_start_pos, a_tag_end_pos) != -1:
                    definition_html = (
                        definition_html[: a_tag_end_pos + 1]
                        + inner_html
                        + definition_html[a_closing_tag_pos:]
                    )
    ...
    def _fix_entry_cross_ref(self, definition_html: str) -> str:
        if definition_html.startswith("@@@LINK="):  # strange special case
            ...
            entry_linked = definition_html[
                len("@@@LINK=") : last_non_whitespace_position + 1
            ]
            return (
                f'<a href="{self._lookup_url_root + entry_linked}">{entry_linked}</a>'
            )
        else:
    ...
    def _fix_sound_link(self, definition_html: str) -> str:
        ...
        while (sound_link_start_pos := definition_html.find("sound://")) != -1:
            ...
            original_sound_link = definition_html[
                sound_link_start_pos:sound_link_end_pos
            ]
            ...
            outer_html_end_pos = definition_html.find("</a>", inner_html_end_pos) + len(
                "</a>"
            )
            definition_html = (
                definition_html[:outer_html_start_pos]
    ...
    def _fix_sound_link(self, definition_html: str) -> str:
        # Template for the new audio element
        autoplay_string = "autoplay"
        sound_element_template = '<audio controls %s src="%s"></audio>%s'

        # Search for <a> elements with 'sound://' links
        while (sound_link_start_pos := definition_html.find("sound://")) != -1:
            sound_link_end_pos = definition_html.find('"', sound_link_start_pos)
            original_sound_link = definition_html[
                sound_link_start_pos:sound_link_end_pos
            ]
            # original_achor_start_pos=original_sound_link.find("<a",)
            # original_achor_end_pos=original_sound_link.find(">")

            # Replace 'sound://' with the appropriate root directory path
            sound_link = original_sound_link.replace("sound://", self._href_root_dir)

            # Find the inner HTML content of the <a> element
            inner_html_start_pos = definition_html.find(">", sound_link_end_pos) + 1
            inner_html_end_pos = definition_html.find("</a>", inner_html_start_pos)
            inner_html = definition_html[inner_html_start_pos:inner_html_end_pos]

            # Find the start and end of the outer <a> element
            outer_html_start_pos = definition_html.rfind("<a", 0, sound_link_start_pos)
            outer_html_end_pos = definition_html.find("</a>", inner_html_end_pos) + len(
                "</a>"
            )
            html_span = definition_html.rfind(
                "<span",
            )

            # Replace the <a> element with the new <audio> element
            definition_html = (
                definition_html[:outer_html_start_pos]
                + sound_element_template % (autoplay_string, sound_link, inner_html)
                + definition_html[outer_html_end_pos:]
            ...
            # Ensure autoplay is only set for the first audio element
            autoplay_string = ""
    ...
    def _fix_img_src(self, definition_html: str) -> str:
        img_tag_end_pos = 0
        while (
            img_tag_start_pos := definition_html.find("<img", img_tag_end_pos)
        ) != -1:
            img_tag_end_pos = definition_html.find(">", img_tag_start_pos)
            img_src_start_pos = definition_html.find(
                ' src="', img_tag_start_pos, img_tag_end_pos
            ) + len(' src="')
            img_src_end_pos = definition_html.find(
                '"', img_src_start_pos, img_tag_end_pos
            )
            img_src = definition_html[img_src_start_pos:img_src_end_pos]
            img_src = self._href_root_dir + img_src.replace("file://", "")
            definition_html = (
                definition_html[:img_src_start_pos]
                + img_src
                + definition_html[img_src_end_pos:]
            )
        return definition_html
    ...
    def clean(self, definition_html: str) -> str:
        ...
        if self._has_styles:
            definition_html = self._expand_compact_html(definition_tml)
